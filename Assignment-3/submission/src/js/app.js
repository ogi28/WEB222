...
function updateTable(observations) {
  ...
  observations.forEach((observation) => {
    ...
    const builtRow = buildRowForObservation(observation);
    addRowToTable(builtRow);
  });
}
...
function start() {
  ...
  const allSpecies = document.querySelector("#show-all");
  const onlyNativeSpecies = document.querySelector("#show-native");
  const onlyIntroducedSpecies = document.querySelector("#show-introduced");

  allSpecies.addEventListener("click", () => showAll());
  onlyNativeSpecies.addEventListener("click", () => showOnlyNative());
  onlyIntroducedSpecies.onclick = () => showOnlyIntroduced();
