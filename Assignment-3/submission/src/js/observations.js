...
function titleCase(s) {
  return s
    .toLowerCase()
    .split(" ")
    .map((cstr) => cstr[0].toUpperCase() + cstr.substr(1))
    .join(" ");
}
...
function filterObservations(observations) {
  return observations.filter(function (observation) {
    return (
      ...
      observation.taxon.native != null &&
      observation.taxon.default_photo.square_url != null &&
