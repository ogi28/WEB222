...
function createImg(src, alt, width) {
  ...
  if (width > 0) width_str = ` width="${width}"`;

  return `<img src="${src.trim()}" alt="${alt.trim()}"${width_str}>`;
}
...
function parseDateString(value) {
  ...
  if (results === null) {
    ...
    let year = parseInt(results[1]); // 0 is full match
    let day = parseInt(results[2]);
    let month = parseInt(results[3] - 1);
...
function validateCoord(lat, lng) {
  return 90 < lat || lat < -90 || lng > 180 || lng < -180 ? false : true;
}
...
function formatCoord(lat, lng, includeBrackets) {
  ...
  return includeBrackets ? `[${lat},${lng}]` : `${lat},${lng}`;
}
...
function formatCoords(...values) {
  ...
  while (valuesCopy.length) {
    lat = valuesCopy.shift();
    if (valuesCopy.length) lng = valuesCopy.shift();
    else throw 'odd number of arguments';
    if (!validateCoord(lat, lng)) throw 'invalid coordinates';
    ary.push('[' + lat + ',' + lng + ']');
  }
...
function generateLicenseLink(licenseCode) {
  //licenseCode.toLowerCase();
  // licenseCode.substring(3, licenseCode.length-1)
  ...
  var dicValue = dictionary[licenseCode];
  if (!dicValue) {
  ...
  var link =
    ...
    licenseCode.toLowerCase().substring(3, licenseCode.length) +
    '/4.0/';

  return link + '">' + dicValue + '</a>';
}
...
function pureBool(value) {
  ...
  var value_type = typeof value; // get value type

  if (value_type === 'boolean') return value;

  if (value_type === 'string') {
    value = value.toLowerCase();
    if (value === 'yes' || value === 'y' || value === 'true' || value === 't') return true;
    if (
      value === 'no' ||
      value === 'n' ||
      value === 'false' ||
  ...
  if (value_type === 'number') return value === 1;
...
function all() {
  ...
  for (var i = 0; (i < arguments.length) & flag; i++) {
    if (pureBool(arguments[i]) === false) flag = false;
  }
...
function none() {
  var flag = true;
  for (var i = 0; i < arguments.length && flag; i++) {
    if (pureBool(arguments[i]) === true) flag = false;
  }
...
function buildUrl(q, perPage, order, license, lat, lng) {
  ...
  if (q.length === 0) throw 'No query';
  if (200 < perPage || perPage < 1) throw 'Wrong number of pages';
  if (order !== 'asc' && order !== 'desc') throw 'Wrong order';
  if (
    ![
      'none',
      'any',
    ...
    throw 'Wrong license';
  if (!validateCoord(lat, lng)) throw 'wrong coord';

  var url =
    'https://api.inaturalist.org/v1/observations?q=' +
