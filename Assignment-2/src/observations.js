...
function observationSummary(data) {
  const r = getResults(data);

  for (let i = 0; i < r.length; i++) {
    console.log(`#${r[i].id} - ${r[i].species_guess} (${r[i].observed_on_details.date})`);
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  if (typeof geoPrivacy === 'string' || geoPrivacy === null) {
    geoPrivacy = geoPrivacy === null ? null : geoPrivacy.toLowerCase();
    var r = getResults(data);
    for (var i = 0; i < r.length; i++) {
      r[i].geoprivacy = r[i].geoprivacy === null ? null : r[i].geoprivacy.toLowerCase();
      if (r[i].geoprivacy === geoPrivacy) {
        arrayRes.push(r[i]);
      }
    }
  }
...
function transformObservation(original) {
  var newObject = {
    ...
    photos: original.photos.map(function (value) {
      return value.url;
    }),
...
function transformObservations(data) {
  var transformArray = [];
  getResults(data).forEach(function (value) {
    transformArray.push(transformObservation(value));
  });
...
function transformObservations2(data) {
  //var transformArray = [];
  var transformArray = getResults(data).map(function (value) {
    return transformObservation(value);
  });
...
/*function getObservationsByPositionalAccuracy(data, options = {}) {
  let r = getResults(data);

  return r
    .filter((value) => {
      if (options.equal) return value.positional_accuracy === options.equal;
      else return true;
    })
    .filter((value) => {
      if (options.min && !options.equal) return value.positional_accuracy >= options.min;
      else return true;
    })
    .filter((value) => {
      if (options.max && !options.equal) return value.positional_accuracy <= options.max;
      else return true;
    });
*/
  function getObservationsByPositionalAccuracy(data, options = {})
  {
    ...
    myarr = results;
    if (options.equal) {
      myarr = results.filter(function (result) {
        return result.positional_accuracy === options.equal;
//if (result.positional_accuracy === options.equal) return result == ture;
      });
    } else if (options.min&& options.max) {
      myarr = data.results.filter(function (val) {
        return val.positional_accuracy <= options.max && val >= options.min;
      });
    } else if (options.min ) {
      myarr = results.filter(function (result) {
      ...
      myarr = data.results.filter(function (val) {
        return val.positional_accuracy <= options.max;
      });
    }
  ...
  r.forEach(value =>
    {
      if (options[0] === ` equal: ${value.positional_accuracy}`)
        return value;
    });
...
function getTaxonPhotos(data) {
  ...
  const sizes = ['original', 'square', 'small', 'medium', 'large'];
  result.forEach((value) => {
    if (value.taxon && value.taxon.default_photo) {
      //console.log(value.taxon.default_photo)
      const photoId = value.taxon.default_photo.id;
      let photoObj = {};
      for (let i = 0; i < sizes.length; i++) {
        photoObj[sizes[i]] = `https://static.inaturalist.org/photos/${photoId}/${sizes[i]}.jpg`;
      }
...
function getUserStats(data) {
  ...
  r.forEach((value) => {
    count++;
    if (value.user.observations_count) observationTotal += value.user.observations_count;
    if (value.user.identifications_count) identificationTotal += value.user.identifications_count;
    if (value.user.species_count) speciesTotal += value.user.species_count;
  });
  ...
  return {
    ...
    totals: {
      ...
      identifications: identificationTotal,
      species: speciesTotal
    },
    averages: {
      observations: observationTotal / count, //observationCount,
      identifications: identificationTotal / count, //identificationCount,
      species: speciesTotal / count // speciesCount,
    }
  };

  //make a loop to go through data to find all the users, store count so I can get average
